...
`yu-picture` 是一个基于 DDD（领域驱动设计）的多人协作图库项目，旨在为用户提供一个高效、安全的图片管理平台。  
...
1. 克隆项目仓库：
   ...
   ```bash
   git clone https://github.com/ecc1esia/yu-picture.git
   cd yu-picture
   ```
...
### P1
- 提供进行 AI文生图/风格迁移 和 AI扩图 两种选择，通过输入提示词生成图像 已接入AI扩图
- 同一空间内 图片向量检索 已实现，等待全流程测试
- 图片向量存储与检索中间件引入 已在服务器端部署 elasticsearch
- 互联网以图搜图api寻找替代品   选择TinEye替换现有百度API
- 前端隐形水印功能实现    未实现

### P2
- 多租户改造（不做，改成使用spaceId实现）
- RBAC 权限控制（不做，改成简单权限控制）
- Session 升级 token(不做)
- 前端 UI/UX 升级
- WebSocket架构升级（暂缓）
- Docker容器化部署
