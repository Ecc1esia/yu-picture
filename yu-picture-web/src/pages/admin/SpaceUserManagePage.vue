<template>
  <div class="spaceUserManagePage">
    <a-flex justify="space-between">
      <h2>空间成员管理</h2>
      <a-space>
        <a-botton type="primary" href="/add_space" target="_blank">+ 创建空间</a-botton>
        <a-button type="primary" ghost href="/space_analyse?queryPublic=1" target="_blank"
          >分析公共图库</a-button
        >
...
const editSpaceRole = async (value: string, record: API.SpaceUserVO) => {
  ...
  if (res.data.code === 0) {
    message.success('编辑成员角色成功')
    fetchData()
  } else {
    message.error('编辑成员角色失败: ' + res.data.message)
  }
}
...
const doDelete = async (id: number) => {
  ...
  if (res.data.code === 0) {
    ...
    message.error('删除成员失败: ' + res.data.message)
  }
}
